...
export type ProjectRecord = { id: string; title: string; slug: string; client: string; summary: string; content: string; services: string; year: string | null; coverImageUrl: string | null; status: "draft" | "published"; sortOrder: number; updatedAt: string };
...
function mapProject(row: Record<string, unknown>): ProjectRecord {
  return { id: String(row.id), title: String(row.title), slug: String(row.slug), client: String(row.client), summary: String(row.summary ?? ""), content: String(row.content ?? ""), services: String(row.services ?? ""), year: row.year ? String(row.year) : null, coverImageUrl: row.cover_image_url ? String(row.cover_image_url) : null, status: row.status as ProjectRecord["status"], sortOrder: Number(row.sort_order ?? 0), updatedAt: String(row.updated_at) };
}
...
export async function getPublishedProjects() {
  ...
  const { data, error } = await supabase
    .from("projects")
    .select("id,title,slug,client,summary,services,year,cover_image_url,status,sort_order,updated_at")
    .eq("status", "published")
    .order("sort_order")
