...
export default async function AdminPortfolioPage({ searchParams }: { searchParams: Promise<{ saved?: string; deleted?: string; error?: string }> }) {
  ...
  return (
    <>
      ...
      {query.error === "featured_limit" ? <div className="admin-config-banner"><span>!</span><div><strong>Límite de destacados alcanzado</strong><p>Solo podés mostrar hasta 3 proyectos en el Home.</p></div></div> : null}
      {query.error === "featured_migration" ? <div className="admin-config-banner"><span>!</span><div><strong>Falta actualizar Supabase</strong><p>Ejecutá la migración de proyectos destacados antes de guardar.</p></div></div> : null}
