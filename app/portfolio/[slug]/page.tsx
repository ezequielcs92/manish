...
export default async function ProjectDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><SiteHeader /><main id="inicio"><article className="case-detail"><header className="container"><Link href="/portfolio">← Volver al portfolio</Link><p>{project.services}</p><h1>{project.title}</h1><div><span>{project.client}</span><span>{project.year}</span></div></header><section className="case-detail-cover">{project.coverImageUrl ? <><Image src={project.coverImageUrl} alt={project.title} fill sizes="100vw" /><i /></> : null}<span>{project.client}</span></section><div className="container case-detail-body"><aside><span>EL DESAFÍO</span><p>{project.summary}</p></aside><div dangerouslySetInnerHTML={{ __html: sanitizeRichText(project.content) }} /></div></article><PageCta eyebrow="¿Construimos el próximo?" title="Tu proyecto empieza acá." /></main><SiteFooter /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData).replace(/</g, "\\u003c") }} /></>;
}
